...
#define __SPI                      MCP2515_DEFAULT_SPI
...
void MCP2515Class::onReceive(void(*callback)(int))
{
  ...
  if (callback) {
    __SPI.usingInterrupt(digitalPinToInterrupt(_intPin));
    attachInterrupt(digitalPinToInterrupt(_intPin), MCP2515Class::onInterrupt, LOW);
  } else {
    detachInterrupt(digitalPinToInterrupt(_intPin));
#ifdef SPI_HAS_NOTUSINGINTERRUPT
    __SPI.notUsingInterrupt(digitalPinToInterrupt(_intPin));
#endif
  }
}
...
{
  __SPI.beginTransaction(_spiSettings);
...
uint8_t MCP2515Class::readRegister(uint8_t address)
{
  ...
  digitalWrite(_csPin, LOW);
  __SPI.transfer(0x03);
  __SPI.transfer(address);
  value = __SPI.transfer(0x00);
  digitalWrite(_csPin, HIGH);
  __SPI.endTransaction();
